...
export class Application {
  ...
  private async onOpenHallOfFameHandler(): Promise<void> {
    ...
    const retailRowHtml = retailResults.items.map(m => `<tr><td>${m.name}</td><td>${m.characterLevel.toString()}</td><td>${m.sessionStartAtUtc}</td><td>${m.sessionEndAtUtc}</td><td>${m.sessionLengthInMs}</td><td>${m.experiencePoints}</td><td>${m.experienceFromKillsPercentage}</td><td>${m.gameVersion}</td><td>${m.numberOfMoves}</td><td>${m.numberOfActions}</td></tr>`);
    const vanillaRowHtml = vanillaResults.items.map(m => `<tr><td>${m.name}</td><td>${m.characterLevel.toString()}</td><td>${m.sessionStartAtUtc}</td><td>${m.sessionEndAtUtc}</td><td>${m.sessionLengthInMs}</td><td>${m.experiencePoints}</td><td>${m.experienceFromKillsPercentage}</td><td>${m.gameVersion}</td><td>${m.numberOfMoves}</td><td>${m.numberOfActions}</td></tr>`);
